			...
			<table id="myTable">
				...
				<tbody>
					...
					<c:forEach var="row" items="<%=result%>" varStatus="loop">
						<tr>
							<td><input type="text" value="${(loop.index)+1}"
								name="member" readonly /></td>
							<!-- 目前跳著勾選店面會有問題 -->
							<td><input type="checkbox" name="cos_storeid${(loop.index)}"
								value="${row.cos_storeid}" /></td>
				...
				<tr>
					<td align="center"><input type="submit" name="proChoiceStore"
						value="Insert"></td>
				</tr>
			</table>
			</div>
